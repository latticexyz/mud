import { Base64, Bytes, Hash, Hex, P256, PublicKey, Signature, WebAuthnP256 } from "ox";

export function recoverPublicKey([a, b]: [
  {
    challenge: Hex.Hex;
  ...
  const payloadA = getPayloadHash({ challenge: a.challenge, metadata: a.signature.metadata });
  const payloadB = getPayloadHash({ challenge: b.challenge, metadata: b.signature.metadata });
  if (!payloadA || !payloadB) throw new Error("could not get payload hash");

  for (const yParity of [0, 1]) {
    const publicKeyA = PublicKey.toHex(
      P256.recoverPublicKey({
        payload: payloadA,
        signature: Signature.from({ ...a.signature.signature, yParity }),
      }),
      { includePrefix: false },
    );
    const publicKeyB = PublicKey.toHex(
      P256.recoverPublicKey({
        payload: payloadB,
        signature: Signature.from({ ...b.signature.signature, yParity }),
      }),
      { includePrefix: false },
    );
    if (Hex.isEqual(publicKeyA, publicKeyB)) {
      return publicKeyA;
    }
  }
}

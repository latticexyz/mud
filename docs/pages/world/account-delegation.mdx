...
The owner of the root namespace (and therefore the `World`) needs to deploy the [`StandardDelegationModule`](https://github.com/latticexyz/mud/blob/main/packages/world-modules/src/modules/std-delegations/StandardDelegationsModule.sol), which includes three delegation control contracts:
...
```solidity copy showLineNumbers {16}
// SPDX-License-Identifier: MIT
pragma solidity >=0.8.21;
...
contract RemoveDelegation is Script {
  function run() external {
    ...
    IWorld world = IWorld(worldAddress);
    vm.startBroadcast(userPrivateKey);
    world.unregisterDelegation(address(0x60A7));
    vm.stopBroadcast();
  }
}
```

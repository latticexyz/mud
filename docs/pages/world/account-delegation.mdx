...
Both [ERC-4337](https://www.erc4337.io/) and delegation allow an account to act on behalf of another account.
However, while there is some overlap, the identity of the delegatee, the account acting on behalf of the original wallet, is different.
In ERC-4337 you create a smart account and all your transactions appear to come from that smart account.
Essentially, that smart contract becomes your identity, and a `World` cannot associate it with the original account.

Our delegation allows any account to be a delegatee.
This means you can continue to use your existing account in a `World`, and just delegate to a session wallet, an [ERC-4337 account contract](https://www.erc4337.io/docs/understanding-ERC-4337/architecture), or any other entity.

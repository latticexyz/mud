...
<Callout type="warning" emoji="⚠️">
  Once a table is created its schema should be considered *immutable*. You could, in theory, modify the schema by
  changing the content of the `store:Tables` table, but doing so corrupts the table and confuses the synchronization
  code, which expects the table's metadata to be consistent.

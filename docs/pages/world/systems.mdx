...
1. Call the function using `SystemSwitch.call`.
   For example, here is how you can call [`IncrementSystem.increment()`](http://localhost:3000/templates/typescript/contracts#incrementsystemsol).
   ...
   Use [`abi.encodeWithSignature`](https://docs.soliditylang.org/en/latest/cheatsheet.html#abi-encoding-and-decoding-functions) to create the calldata.
   The first parameter is [the function name with the parameter types](https://docs.soliditylang.org/en/v0.8.23/abi-spec.html#function-selector).
   If the function gets any parameters, they are specified as extra parameters.
   ...
   Using `SystemSwitch.call` with the calldata created by `abi.encodeWithSignature`.

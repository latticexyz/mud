...
<Callout type="info" emoji="ℹ️">
  Sometimes you think you need to call a `System` from another, when there is no reason to have separate `System`s at
  all. [See here](/guides/best-practices/dividing-into-systems) for a discussion of when it makes sense to have a single
  `System` and when splitting the code makes sense.
</Callout>

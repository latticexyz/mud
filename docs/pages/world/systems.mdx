...
<Callout type="error" emoji="🚫">
  ...
  but you should only do it if it's necessary. A root `System` acts as the `World`, so a `delegatecall` from a root `System` behaves exactly like a `delegatecall` from the `World`, and gives full access to the `World` to the called contract.
  However, any contract you `delegatecall` inherits your permissions, in this case unlimited access to the `World` and

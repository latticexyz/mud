...
The [`System`s](./systems) can be upgraded without changing the underlying `World` code, using [`mud deploy --worldAddress`](http://localhost:3000/cli/deploy#upgrading-a-world).
...
```typescript filename="mud.config.ts" copy showLineNumbers {12-14}

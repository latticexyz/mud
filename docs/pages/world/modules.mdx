...
Internally, it works by installing a [hook](http://localhost:3000/store/advanced-features#storage-hooks) that maintains:
...
It installs a [System](/world/world-101#systems) that returns all records in a given table, with offset-based pagination. This requires the `KeysInTable` module to be installed on that table.

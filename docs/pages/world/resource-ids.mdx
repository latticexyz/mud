...
```ts
type resourceType = "tb" | "ot" | "ns" | "sy" | "md"
type resource = {
  type: resourceType;
  namespace: string;
  name?: string;
}
type resourceId = BigInt

function num2bytes(id : resourceId) : Array<Number> {
  if (id < 0x100n)
    return [Number(id)]
  else
    return num2bytes(id / 256n).concat(Number(id % 0x100n));
}

function resourceIDtoString(id : resourceId) : resource {

  for(let i=31; i>=0; i--)
    bytes[31-i] = number(id & (0xFF << (i*8))
  return {type: "tb", namespace: "hello"}
}
```

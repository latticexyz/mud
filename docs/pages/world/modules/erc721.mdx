...
This gives us the namespace `DDDD`, which is easy to recognize both as hex and as text, because `0x44` is the [ASCII representation](https://en.wikipedia.org/wiki/ASCII#Character_set) of the letter `D`.
...
```solidity filename="ManageERC721.s.sol" copy {29-34, 42-58}
...
Then we use `ERC721Registry` to get the token address.

...
Before you run `mud verify` you need to create a [Foundry profile](https://book.getfoundry.sh/reference/config/overview).

1. Change to the `packages/contract` directory and edit `foundry.toml` to add a profile with the `eth_rpc_url` for your network (if needed).
...
Note that the blockchain has to be one that the verifier supports.
...
1. These are the command line options you can specify on `mud verify`:
   ...
   | - `--verifier`   | URL to use to access the verifier's API                        | string  | depends on the verifier used |

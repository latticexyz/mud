...
<Aside emoji="ℹ️">
  **ECS mental model**  
  In order to trigger encounters we need to recognize when a player (an entity that can enter encounters) is walking through tall grass (an entity that can generate encounters).  
  
  We then need the game to enter into an entirely different state—the encounter (itself an entity)—in which the player loses the ability to move.  
  
  Afterwards, we would need to enable to the player to capture an emojimon (an entity capable of being captured and owned).
</Aside>


Before continuing, try figuring out what components and systems would need to be added to get the build all these features. You could even try building them—we've already taught you all that is needed (and you can view the gif in the [Introduction](https://mud.dev/tutorials/emojimon) as a reference).
...
```
Then let's update the spawn method in `MapSystem.sol` to include the `Encounterable` table.
...
But to start an encounter from tall grass, our app needs to know when tall grass is being moved through. Let's add a query in the `MapSystem.sol`'s move method to handle this.
...
<Aside emoji="ℹ️">
  If you're interested in generating somewhat robust randomness in your application, you can look into [Chainlink VRF](https://docs.chain.link/vrf/v2/introduction/) and stay tuned for future MUD tooling.
</Aside>
...
Let's also make sure these interactions are rendering optimistically like everything else. Let's update `createSystemCalls` accordingly.
...
Then let’s query to see if the player is in an encounter and, if so, get the monster type and render the `EncounterScreen`. We'll add this to `GameBoard.tsx`.

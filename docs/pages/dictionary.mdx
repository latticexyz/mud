...
The functionality of [state variables](https://docs.soliditylang.org/en/latest/internals/layout_in_storage.html) is implemented using [tables](/store/tables).
...
An easy workaround is to use the hash of the string as the key instead of the string itself.
...
<Callout type="info" emoji="📝">
  The example files here are using [the React template](https://github.com/latticexyz/mud/tree/main/templates/react).
  However, the technique is identical for any MUD application.
</Callout>
...
1. In the `mud.config.ts` file create an [offchain](/store/tables#types-of-tables) singleton table.
   ...
   ```typescript filename="mud.config.ts" copy {15-22}
   import { defineWorld } from "@latticexyz/world";
...
1. Add code to emit the event to any `System` call that needs it.
   ...
   ```solidity filename="TasksSystem.sol" copy {5,11}
   // SPDX-License-Identifier: MIT
   pragma solidity >=0.8.24;

   import { System } from "@latticexyz/world/src/System.sol";
   import { Tasks, Event, TasksData } from "../codegen/index.sol";
...
1. The client-side registration depends on the state library used.

   - **RECS**

     Use `components.Event.update$.subscribe`.

     ```typescript
     components.Event.update$.subscribe((update) => {
       console.log(update);
     });
     ```

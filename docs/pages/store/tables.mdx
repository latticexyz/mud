...
When you just need to store a single value (for example, the world map or the name of an in-game currency), you use a singleton table, a table whose key schema is empty.
With an empty key schema there is just one row.

```solidity copy
SchemaType[] memory _keySchema = new SchemaType[](0);
Schema keySchema SchemaLib.encode(_keySchema);
```

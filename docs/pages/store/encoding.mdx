...
Each table has two schemas: a key schema and a value schema.
The schemas define the data type of each column.

The key schema and the value schema use the same data structure, a `bytes32` value that encodes the total byte length of all static length fields, the number of static length fields, the number of dynamic length fields, and up to 28 column types.

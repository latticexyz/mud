...
Use [schema-type package](https://github.com/latticexyz/mud/tree/main/packages/schema-type) to work with it directly.
It's needed because `abi.decode` requires statically hardcoded types, and ABI encoding has very big blobs.
...
- PackedCounter is lengths to tell arrays apart, packed tightly in 1 bytes32 word.

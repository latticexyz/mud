...
<Params>
  ...
  <Param name="tables">
    ...
    <Params title="Table options">
      ...
      <Param name="schema">
        A mapping of field names to their schema type, `enums` label, or `userTypes` label.
        Note that static-length fields such as `uint256` have to appear before dynamic-length fields such as `string` or `uint8[]`.
      </Param>
      <Param name="key">

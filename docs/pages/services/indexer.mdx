...
### Read data from SkyStrife season 0

Here we can use the existing Redstone testnet indexer.

1. Read the data from the indexer.

   ```sh copy
   INDEXER_URL=https://indexer.holesky.redstone.xyz/trpc
...
1. Get the match names.

   ```sh copy
   echo $TABLES | jq '.[] | select (.name=="MatchName")'
   ```

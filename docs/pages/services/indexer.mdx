...
### Schemaful indexing to SQLite via pnpm
...
### Schemaless indexing to PostgreSQL via pnpm
...
1. Run the indexer frontend, which is responsible for serving the data to the client, in a separate command-line window.
...
### Schemaful indexing with PostgreSQL via pnpm
...
1. Get the tasks.
   ...
   - Read all the data and filter it locally.
     ...
     cat tables.json | jq -c '.[] | select (.name=="Tasks")' > tasks-table.json
     cat tasks-table.json | jq .records | more
     ```

     You can also get additional table information, for example the `tableId`:

     ```sh copy
     cat tasks-table.json | jq .tableId
     ```

     To see all the tables in the data, you can use:

     ```sh copy
     cat alldata.json | jq '.[0].result.data.json.tables[].name'
     ```

   - Ask the indexer for only the information in a specific table (which requires you to know the `tableId` for it).

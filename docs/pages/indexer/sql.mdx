...
<Tabs items={["curl queries", "Typescript selectFrom"]}>
  <Tab>
    You can run SQL queries by communicating directly with the indexer's API, for example using [curl](https://curl.se/).
    To do so: 
  ...
  <Tab>
    Even if you choose not to use MUD client synchronization, you can query the indexer from [Typescript](https://www.typescriptlang.org/), using the same `selectFrom` you can also use to create [initial hydration filters](#mud-state-hydration-via-sql-api).

   <Callout type="warning">

    This API is still experimental and may change.
    ...
    1. Add the package that includes the SQL interface library.
    ...
    1. Replace `src/main.ts` with this file.
        ...
        The queries supported by `selectFrom` are a subset of those SQL supports.
        The results here come from a single table, and only `WHERE` and `LIMIT` clauses are supported "natively"
        ...
        Show the SQL query and the actual records.
...
The SQL API does not support `SELECT * FROM <table>`, you _have_ to specify column names.

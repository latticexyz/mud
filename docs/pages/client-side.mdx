...
**Note**: when using the default MUD templates (created with `pnpm create mud@next`), the Store data will be available in **both** client-side stores. You are free to query with the one that makes the most sense.

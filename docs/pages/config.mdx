...
The `System`s are defined implicitly, as all the contracts that match `src/namespaces/app/systems/*System.sol`.
...
In this configuration, we explicitly specify `RootSystem` so we can specify that access to it is limited to authorized addresses.
...
- `src/namespaces/app/systems` - `System`s that belong to the `app` namespace.
- `src/namespaces/config/systems` - `System`s that belong to the `app` namespace.

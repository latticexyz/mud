...
This configuration file creates a single namespace, `app`.
Within that namespace, there is one table, `app__Tasks`.
...
- `src/namespaces/<namespace>/systems` - `System`s that belong to this namespace.
- `src/namespaces/<namespace>/codegen/tables` - the generated code for tables that belong to this namespace.
- `src/namespaces/<namespace>/codegen/index.sol` - a single file that imports all the table definitions.

...
3. `deploy-v2`: deployer for MUD v2 projects that deploys an app using the [World](world) kernel onchain 
...
When using the deployer, you must set the private key of the deployer using the `PRIVATE_KEY` environment variable. You can make this easier by using `[dotenv](https://www.npmjs.com/package/dotenv)` before running `mud deploy-v2` in your deployment script.

To set the profile used by the deployer, either set your `FOUNDRY_PROFILE` environment variable, or pass `--profile <profileName>` to the deployer (eg: `mud deploy-v2 --profile optimism-mainnet`).

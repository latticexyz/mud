...
When using the MUD templates, the project is laid down as a monorepo with contracts in the `packages/contracts` folder, and the client in the `packages/client`. `dev-contracts` need to be executed in the folder with the MUD config -- `packages/contracts` -- and point to where the client codegeneration should be saved. In this casem the `packages/client/src/mud`.

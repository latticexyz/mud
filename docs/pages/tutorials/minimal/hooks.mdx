...
1.  Create a contract with the functions to `hang` on the hooks.
    ...
    Note that MUD calls either `onSetRecord` (before the change) or both `onBeforeSetField` and `onAfterSetField`.
...
We need the world contract twice, so we'll just store it in a variable.
...
Register the `CounterHook` contract as a hook for the `Counter` table.

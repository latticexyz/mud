...
**Note:** `pnpm dev` does not redeploy the contracts when you change this file, so you'll need to start it manually.

Now let’s render the terrain in the client via `GameBoard.tsx`.

<CollapseCode>

```tsx filename="packages/client/src/GameBoard.tsx" {5-7,12-13,31-46,51-53} copy showLineNumbers
...
Because we are changing `MapSystem.sol`, the `pnpm dev` process will redeploy the contracts, and run the updated script.

...
```solidity filename="packages/contracts/script/PostDeploy.s.sol" {7-8,18-58} copy showLineNumbers
...
**Note:** `pnpm dev` does not redeploy the contracts when you change this file, so you'll need to restart it manually.

Now let’s render the terrain in the client via `GameBoard.tsx`.

<CollapseCode>

```tsx filename="packages/client/src/GameBoard.tsx" {5-7,12-13,31-46,51-52} copy showLineNumbers
...
```solidity filename="packages/contracts/script/PostDeploy.s.sol" {7-9,57-61} copy showLineNumbers
// SPDX-License-Identifier: MIT
...
Because we are changing `MapSystem.sol`, the `pnpm dev` process will redeploy the contracts, and run the updated `PostDeploy.s.sol` script.

<CollapseCode>

```solidity filename="packages/contracts/src/systems/MapSystem.sol" {7,14-15,29-30} copy showLineNumbers
// SPDX-License-Identifier: MIT
pragma solidity >=0.8.21;
...
```ts filename="packages/client/src/mud/createSystemCalls.ts" {11,13-15,22-25,65-68} copy showLineNumbers
...
Note that this function is not the most efficient possibility.
...
```solidity filename="packages/contracts/src/systems/MapSystem.sol" {14-17,34-37} copy showLineNumbers
// SPDX-License-Identifier: MIT
pragma solidity >=0.8.21;
...
```ts filename="createSystemCalls.ts" {14-23,29,34-38,68,78-82} copy showLineNumbers
qbzzt@Oris-MacBook-Pro emojimon % cat packages/client/src/mud/createSystemCalls.ts
import { Has, HasValue, getComponentValue, runQuery } from "@latticexyz/recs";
import { ClientComponents } from "./createClientComponents";
import { SetupNetworkResult } from "./setupNetwork";
import { singletonEntity } from "@latticexyz/store-sync/recs";

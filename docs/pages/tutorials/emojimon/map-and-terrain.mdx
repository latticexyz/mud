...
```solidity filename="packages/contracts/src/systems/MapSystem.sol" {7,14-15,30-34} copy showLineNumbers
// SPDX-License-Identifier: MIT
pragma solidity >=0.8.21;
...
```ts filename="packages/client/src/mud/createSystemCalls.ts" {11,13-15,31-32,60-61} copy showLineNumbers
...
```ts filename="createSystemCalls.ts" {13-22,39-41,62} copy showLineNumbers
import { Has, HasValue, getComponentValue, runQuery } from "@latticexyz/recs";
import { ClientComponents } from "./createClientComponents";
import { SetupNetworkResult } from "./setupNetwork";
import { singletonEntity } from "@latticexyz/store-sync/recs";

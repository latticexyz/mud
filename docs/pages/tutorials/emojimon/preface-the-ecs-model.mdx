...
- **Entities** are the basic units of a game, and they can have any number of components.
  In a typical MUD implementation each entity is assigned a unique 256 bit identifier that is used as a key to the component tables (see next item).
...
In the scope above there are a few key interactions that we need to account for. Let's use a basic one to as an example, **the interaction between a player moving and a boulder blocking its movement.**

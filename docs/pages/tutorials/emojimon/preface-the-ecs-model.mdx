...
To apply the ECS model for the example above you'd use `uint256` as an entity identifier, and create both a `movable` table and an `obstruction` table, both of type `bool` (for boolean values).

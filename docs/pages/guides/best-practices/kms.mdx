...
1. Set the AWS environment variables.
   ...
   | AWS_ENDPOINT_URL      | [See the AWS console](https://docs.aws.amazon.com/general/latest/gr/kms.html) for your region |
...
1. Update `PostDeploy.s.sol` script. These scripts are usually incompatible with the KMS deployer as they rely on reading a private key from the local development environment. To ensure `forge` uses the KMS signer, the script should use `vm.startBroadcast()` with no arguments.

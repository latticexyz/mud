...
In MUD access is granted by Ethereum address, every non-root `System` has a different one.
So if only some of the functions of a `System` need to have certain privileges, it might make sense to put just those functions in a separate `System`.

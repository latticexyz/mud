...
# Permissionlessly Extending a `World`

One of the great features of blockchains is permissionless extensibility, the ability of unrelated developers to extend an application without cooperation or approval from the original developers.
In this tutorial you extend the `Counter` example to allow users to leave a message while incrementing the counter.

## The required resources for an extension
...
1. To have a `World` with the `Counter` example to modify, go to a command line window and create a blockchain with a `World` using [the TypeScript template](/template/typescript/getting-started) and start the execution.
   Choose either the `vanilla` user interface or the `react-ecs` one.
...
1. The easiest way to create the Solidity code is to use the MUD template.
   Create a new MUD application.
   Use the `vanilla` template.
   Run these commands from the same directory as the previous `pnpm create` command but in a separate command line window.
...
1. Edit `mud.config.ts` to include the definitions we need.
   ...
   ```ts filename="mud.config.ts" copy showLineNumbers {4,6-13,16-19}
...
1. Create this script in `script/MessagingExtension.s.sol`.
   ...
   [Register the namespace](https://github.com/latticexyz/mud/blob/main/packages/world/src/modules/init/implementations/WorldRegistrationSystem.sol#L35-L66).
...
1. Run the script.
   ...
   ```sh copy
   forge script script/MessagingExtension.s.sol --rpc-url http://127.0.0.1:8545 --broadcast
   ```
...
1. Increment and write a message.
   ...
   ```sh copy
   source .env
   cast send $WORLD_ADDRESS --rpc-url http://127.0.0.1:8545 --private-key $PRIVATE_KEY "messaging__incrementMessage(string)" "hello"
   ```

   When a function is _not_ in the root namespace, it is accessible as `<namespace>__<function name>` (as long as it is [registered](https://github.com/latticexyz/mud/blob/main/packages/world/src/modules/init/implementations/WorldRegistrationSystem.sol#L180-L218)).
...
1. Start another command line interface in the same directory (`extension/packages/client`).
...
1. The network configuration does not have the correct `World` address.
   ...
   Note: If you followed the directions above to create `extendMe`, the `World` address is `0xf599e8f01a5ca469cda10711c3f2ffa4eeed755e`.

   ```json filename="worlds.json" copy {3}
   {
     "31337": {
...
1. Edit `src/index.ts` to comment out the counter update.

   <CollapseCode>

   ```ts filename="index.ts" copy showLineNumbers {11,17}
...
We still don't have the `Counter` value available.
...
1. Edit `src/mud/setupNetwork.ts` to add a `tables` parameter with the `Counter` object to the `syncToRecs` call.
   ...
   ```ts filename="setupNetwork.ts" copy showLineNumbers {36,91}
   /*
...
1. Edit `src/index.ts` to re-enable the counter update.

   <CollapseCode>

   ```ts filename="index.ts" copy showLineNumbers {10-15}
   import { setup } from "./mud/setup";
   import mudConfig from "contracts/mud.config";
...
1. Now you can click **Increment** and see the update.

...
1. Edit `mud.config.ts` to include the definitions we need.
   You could also delete the counter definitions (lines 6-11), but it doesn't matter.

   ```ts filename="mud.config.ts" copy showLineNumbers {4,12-18,20-25}
   ...
   export async function setupNetwork() {
     ...
     */
     const { components, latestBlock$, storedBlockLogs$, waitForTransaction } = await syncToRecs({
       ...
       address: networkConfig.worldAddress as Hex,
       publicClient,
       tables: resolveConfig(counterConfig).tables, GOON

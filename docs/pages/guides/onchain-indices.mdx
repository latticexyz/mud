...
However, more complex applications require transformations of onchain data that would be infeasible to compute onchain. For example, _“which players are at this position?”_, or _“how many gold mines does Bob own?_.
...
However, we only know onchain the position of a given player, not which players are at each position; there is no reverse mapping. It’s time to add an index!

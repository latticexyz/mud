...
However, we often require complex transformations of onchain data that is infeasible to compute onchain. For example, _“which players are at this position?”_, or _“how many gold mines does Bob own?_.
...
One way to know if a position is occupied is to have a list of which players are there, and check if that list is empty.
...
Having a list of players at each position is useful, but ultimately we only want to know if _some_ player is at the position, not their specific addresses. Therefore, we can take advantage of managing this in our own contracts and write a simpler index.
...
import { Position, PlayersAtPosition } from "../codegen/index.sol";
...
```solidity filename="MoveSystem.sol" copy showLineNumbers {5,6,12-13}
// SPDX-License-Identifier: MIT
pragma solidity >=0.8.24;

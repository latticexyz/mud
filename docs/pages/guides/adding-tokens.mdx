...
1. Scroll up, get the address for `RewardSystem`, and check that balance.

   ```sh copy
   cast call $TOKEN_ADDRESS "balanceOf(address)" 0x7a2088a1bFc9d81c55368AE168C2C02570cB814F | cast --from-wei
   ```
...
1. Check the balances for `$MY_ADDRESS` and `RewardSystem`.
...
1. Back in the original application, edit `packages/contracts/src/systems/TasksSystem.sol`.

   <CollapseCode>

   ```solidity filename="TasksSystem.sol" showLineNumbers copy {17-18}

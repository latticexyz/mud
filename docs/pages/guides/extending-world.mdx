...
1. Create a new MUD application.
   Use either the `vanilla` template or the `react-ecs` one.
...
1. To have a `Counter` example `World` to modify, go to a separate command line window and create a blockchain with a `World` using [the TypeScript template](/templates/typescript/getting-started) and start the execution.

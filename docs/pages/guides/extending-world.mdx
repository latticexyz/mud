...
1. Increment and write a message.
   ...
   ```sh copy
   source .env
   cast send $WORLD_ADDRESS --private-key $PRIVATE_KEY "messaging_MessageSystem_incrementMessage(string)" "hello"
   ```
...
1. Open the application at [http://localhost:3001](http://localhost:3001).
   At this point the server gives us web pages, but the JavaScript in them does not work, the next steps are to fix it.

1. Because `Counter` is not defined in the extension's `mud.config.ts`, it isn't one of the components.
   Comment out the lines that register a handled for updates on `components.Counter` in `src/index.ts`.

   <CollapseCode>

    ```ts fileName="index.ts" copy showLineNumbers {10-17}
...
1. The network configuration does not have the correct `World` address. 
   ...
   By default, you can do see this application on URL [http://localhost:3000](http://localhost:3000).

   Note: If you have not modified the `contracts` package of the application you extend, the address is `0x6e9474e9c83676b9a71133ff96db43e7aa0a4342`.

   Edit `src/mud/getNetworkConfig.ts` to use the correct address.

   <CollapseCode>

    ```ts fileName="getNetworkConfig.ts" copy showLineNumbers {22-26}
    /*
    ...
    */
    ...
    export async function getNetworkConfig() {
      ...
      */
      const chainId = Number(params.get("chainId") || params.get("chainid") || import.meta.env.VITE_CHAIN_ID |$
...
Also, we don't have access to the `Counter` component, so we comment out the code that uses it.


   <CollapseCode>

    ```ts fileName="createSystemCalls.ts" copy showLineNumbers {43-44,46}
    /*
...
![The extended UI with the messages shown in the MUD Dev Tools](../world/extend-ui-with-msgs.png)

### Read the counter

We still don't have the `Counter` value available, because it's not in our `mud.config.ts`.
...
1. Copy the `mud.config.ts` to `src/mud/counter.config.ts`.
...
1. Edit `src/mud/setupNetwork.ts` to add a `tables` parameter with the `Counter` object to the `syncToRecs` call.
   ...
   <CollapseCode>
    ...
    ```ts fileName="setupNetwork.ts" copy showLineNumbers {17,28,84}
    /*
...
1. Now that `Counter` is available again, undo the edits that removed it.
   ...
   - `src/mud/createSystemCalls.ts`
     ...
     <CollapseCode>
      ...
      ```ts filename="createSystemCalls.ts" copy showLineNumbers {45}
      /*
      * Create the system calls that the client can use to ask
      * for changes in the World state (using the System contracts).
      */
      ...
      export function createSystemCalls(
        ...
        */
        ...
        const increment = async () => {
          ...
          * on the World contract.
          */
          const tx = await worldContract.write.messaging_MessageSystem_incrementMessage([`message at ${Date()}`]);
          await waitForTransaction(tx);
          return getComponentValue(Counter, singletonEntity);
        };

...
1.  Create this script in `packages/contracts/scripts/DeployDelegation.s.sol`
...
import { StandardDelegationsModule } from "@latticexyz/world-modules/src/module$
...
1. Create `scripts/TestDelegation.s.sol`.
...
1. In the user interface click a few buttons.
   This step is important because there is always going to be a call by `USER_ADDRESS`, it is created by the post deploy script (`script/PostDeploy.s.sol`).
   We want to make sure we can see that the time of the last call from `USER_ADDRESS` is now _after_ the other calls.
...
1. See that the call from `USER_ADDRESS` is now at the bottom, it is the latest call.
   This call was executed by `USER_ADDRESS_2`.
...
- Add the ABI for the functions whose calldata needs to be created and provided as a parameter.
  ...
  - `LastCallSystem.newCall`

  The ABIs are located under `packages/contracts/out`.

- Provide the values of the constants we need.
  Both of those constants are `System` identifiers, so their format is:
  `sy + <namespace in 14 bytes> + <system name in 16 bytes>.`
...
- Create the functions for:
  ...
  - Delegating access with `registerDelegation`
  - Usign delegated access with `callFrom`
...
```typescript filename="App.tsx" copy showLineNumbers {1,8-60,119-149,176,188-211}

...
- `Store_SpliceDynamicData` modifies a subset of the bytes that represents the encoded values of all dynamic (variable length) fields for a particular key tuple in a particular table. It includes some additional fields on the static splice event.

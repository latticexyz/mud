...
In this tutorial you add a table of historical counter values and the time in which the counter reached those values, after the `World` is already deployed.
...
1. Create a new package.
   This step is necessary because if we modify the `contracts` package the `pnpm dev` process detects the changes and redeploys the `World`, which is not the behavior we want here.
...
1. Edit `src/mud/setupNetwork.ts` to add a `tables` parameter to the `syncToRecs` call.
   ...
   ```ts filename="setupNetwork.ts" copy showLineNumbers {9,27,83}
   /*

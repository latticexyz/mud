...
You need to edit both `supportedChains.ts` and `setupNetwork.ts`.
...
- Very often chain objects in Viem don't have a webSocket endpoint, only an HTTP one.
  Edit `packages/client/src/mud/setupNetwork.ts` not only use a webSocket endpoint when one is defined.
...
For example, to connect to the Holesky `World` at address [`0x32b4156e9b073706608e3b522e3372c94efe7640`](https://eth-holesky.blockscout.com/address/0x32b4156e9b073706608e3b522e3372c94efe7640?tab=contract_code), which contains [the standard contract code](/templates/typescript/contracts), you use this URL:

```url
http://localhost:3000/?chainid=17000
```

And this file in `worlds.json`:
...
```json
{
  ...
  "17000": {
    "address": "0x32b4156e9b073706608e3b522e3372c94efe7640",
    "blockNumber": 1782097
  }
}
```

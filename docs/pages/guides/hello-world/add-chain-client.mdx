...
- Very often chain objects in Viem don't have a webSocket endpoint, only an HTTP one.
  ...
  ```ts showLineNumbers {1-18,22}
  const urlTypes =
    (networkConfig.chain.rpcUrls.default.webSocket ? "w" : "-") +
    (networkConfig.chain.rpcUrls.default.http ? "h" : "-");
  let fallbacks = [];
  switch (urlTypes) {
    case "wh":
      fallbacks = [webSocket(), http()];
      break;
    case "w-":
      fallbacks = [webSocket()];
      break;
    ...
    case "--":
    default:
      console.log("No URL type I can use for the chain");
  }

...
1. Run `anvil` manually.
   ...
   ```sh
   anvil -b 1 --block-base-fee-per-gas 0 | tee anvil.log
   ```
...
1. Run `pnpm dev` normally.
   ...
   Note that because of the way things work, there might be multiple copies of the same `console.log` message.

#### MUD devtools

You can also see what is happening from your application by enabling to MUD devtools.
...
In the minimal application you get from `pnpm create mud@<version> <app>` this is already included.

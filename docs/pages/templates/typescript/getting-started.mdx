...
1. Start the development server, which updates automatically when you modify files.
   ...
   You may receive the error message: `[vite] Internal server error: Failed to resolve import "contracts/out/IWorld.sol/IWorld.abi.json" from "src/mud/setupNetwork.ts"`.
   If so, ignore it.

## Quick start

Let's create your first project with MUD!

### Prerequisites

Make sure you have the following dependencies installed.

- Install [Node.js](https://nodejs.org/en/download/) v18
- Install [pnpm](https://pnpm.io/installation#using-npm) with `npm install -g pnpm`
- Install [Foundry](https://book.getfoundry.sh/getting-started/installation)

> **Note for Windows users**: You may have additional steps/prerequisites to install things like Foundry. We're still working on docs for these.

### Create a new project

It's easy to spin up a new project with MUD. Just run the following command:

```
pnpm create mud@canary your-project-name
```

You'll be prompted to pick the type of template to use (vanilla JS or React).

### Run your project

Jump into your new project's directory and run it with:

```
pnpm run dev
```

Under the hood, this will do a few different steps:

- Generates code to make it easier and safer to interact with MUD
- Starts a local Ethereum node with anvil (from Foundry)
- Deploys contracts to the local node
- Starts the client app

## Project layout

TODO

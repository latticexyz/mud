...
- `bytes32 table`, which is a single 32 byte value that contains both the namespace and the tableID.
...
- `uint8 schemaIndex`, the number in the data schema of the relevant field.
  This parameter only appears is only the field specific event, `StoreSetField`.
...
- `StoreSetField(bytes32 table, bytes32[] key, uint8 schemaIndex, bytes data)`, when one of the data fields in the schema is changed.
- `StoreDeleteRecord(bytes32 table, bytes32[] key)`, when a record is deleted.
- `StoreEphemeralRecord(bytes32 table, bytes32[] key, bytes data)`, when a record is created in an [ephemeral table](/store/advanced-features#ephemeral-tables).
  Ephemeral tables are not stored onchain, which makes them a lot cheaper.
...
The first word is the table.
It is two ASCII strings, one in byte 0 and one in byte 16.
...
The second string is `Tables`, so we are creating a record in the `Tables` table (adding a table).
...
But for dynamic fields, whose length is unknown, the standard is the encode a 32 byte word with the offset to where the field starts.
...
So the first two fields are the key and value schema, respectable, and they are one 32-byte word each.

...
1. Create these files in a previously empty directory:
   ...
   ```json filename="package.json" showLineNumbers copy
   {
     ...
     "type": "module",
     "scripts": {
       "indexerClient": "npx ts-node --esm index.ts"
     },
     "dependencies": {
   ...
   ```ts filename="index.ts" showLineNumbers=4 copy
...
1. Run the client
   ...
   If you are running [the minimal template](/tutorials/minimal), the expected output should be similar to:

   ```
   qbzzt1@penguin:~/clnt2$ pnpm indexerClient

   > client@0.0.0 indexerClient /home/qbzzt1/clnt2
...
| Field                  | Meaning                                                                                       |
| ---------------------- | --------------------------------------------------------------------------------------------- |
| id                     | The table's ID, which encodes the `World` address, the name space, and the name of the table. |
| address                | The `World` address                                                                           |
| tableId                | The ASCII encoded name of the namespace and then table name                                   |
| namespace              | The namespace of the table                                                                    |
| name                   | The name of the table                                                                         |
| keySchema              | The schema for the key                                                                        |
| valueSchema            | The schema for the value                                                                      |
| lastUpdatedBlockNumber | The data is correct as of that block (it could change later)                                  |

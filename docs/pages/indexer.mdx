...
Before you build the software with `pnpm install` and `pnpm build`.
...
1. Build the repository [as explained here](contribute#local-development-setup).
   ...
   **Note**: While MUD v2 is in alpha, there might be breaking changes between pre-release versions.
   ...
   There are multiple ways to do this, but an easy way is to go to [our tags page](https://github.com/latticexyz/mud/tags) and download the repository from there.
   Once you do that, unzip it and proceed with `pnpm install` and `pnpm build` normally.
...
### Using the indexer with recs (the MUD template code)

To use the indexer with the [MUD template code](tutorials/minimal), or with anything else that uses [the recs `store-sync`](https://github.com/latticexyz/mud/blob/main/packages/store-sync/src/recs/syncToRecs.ts), you specify `indexerUrl` in the `syncToRecs` call.
In the template this call is located [in src/mud/setupNetwork.ts](https://github.com/latticexyz/mud/blob/main/templates/vanilla/packages/client/src/mud/setupNetwork.ts#L77-L83).
If you run the indexer locally, you can replace it with:
...
const { components, latestBlock$, blockStorageOperations$, waitForTransaction } = await syncToRecs({
  ...
  indexerUrl: "http://localhost:3001/trpc",
});
```

Note that this is code that runs _in the browser_.
If you run the browser in a different location from the indexer, you need to change `localhost` to the appropriate hostname.

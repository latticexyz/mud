...
The queries above give us a snapshot, the situation at a specific block.
For synchronization we want to get a continuous flow of information with the changes, and for that you use _live queries_.

The syntax is slightly different. The request needs to use [the `GET` method](https://developer.mozilla.org/en-US/docs/Web/HTTP/Methods), so the parameters (address and query) are provided on the URL. The easiest way to do this is with the `--data-urlencode` parameter.

```sh copy
curl https://dozer.mud.garnetchain.com/q-live -G --compressed  \
  --data-urlencode "address=0x95F5d049B014114E2fEeB5d8d994358Ce4FFd06e" \
  --data-urlencode "query=SELECT description, createdAt FROM app__Tasks ORDER BY createdAt"  \
  -H 'Accept-Encoding: gzip' \
  -H 'Content-Type: application/json'
```
...
```
data: {"block_height":6652149,"result":[[["description","createdat"],["","0"],["Walk the dog","1723495628"],["Take out the trash","1723495640"],["Do the dishes","1723495642"],["Learn Elvish","1723688332"],["Invade Narnia","1723688340"]]]}

data: {"block_height":6652150,"result":[[]]}

data: {"block_height":6652151,"result":[[]]}

data: {"block_height":6652152,"result":[[]]}

data: {"block_height":6652153,"result":[[]]}

data: {"block_height":6652154,"result":[[]]}

data: {"block_height":6652155,"result":[[["description","createdat"],["Create a todo list","1725474162"]]]}

data: {"block_height":6652156,"result":[[]]}

data: {"block_height":6652157,"result":[[]]}

data: {"block_height":6652158,"result":[[]]}

data: {"block_height":6652159,"result":[[]]}
```

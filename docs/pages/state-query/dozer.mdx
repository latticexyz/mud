...
You do not need these steps to run `curl` queries, but you do need this client to see how to [modify a client to support dozer](#using-dozer-from-a-mud-client).
...
1. To see the content of the `app__Creator` table, edit `mud.config.ts` to add the `Creator` table definition.
...
To update the client, you change `src/mud/setupNetwork.ts` to:
...
We can use the same filters for the live synchronization, but hydration is the part we need to speed up typically, so it's OK to just synchronize every table change from now on.

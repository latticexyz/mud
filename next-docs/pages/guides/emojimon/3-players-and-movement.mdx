...
export const GameBoard = () => {
  ...
  return <GameMap width={20} height={20} 
            onTileClick={canSpawn ? spawn : undefined} players={player ? [player] : []} 
         />;
};
```
...
```tsx filename="packages/client/src/mud/createClientComponents.ts" {1,9-10} showLineNumbers copy
...
Copy the code below and replace the existing `moveBy` method in `createSystemCalls.ts`.

<CollapseCode>

```tsx filename="packages/client/src/mud/createSystemCalls.ts" {23-32,35-37,50-61,64-67} copy showLineNumbers
import { Has, HasValue, getComponentValue, runQuery } from "@latticexyz/recs";
import { uuid } from "@latticexyz/utils";
import { ClientComponents } from "./createClientComponents";
import { SetupNetworkResult } from "./setupNetwork";
...
Overrides have an identifier that allows for them to be deleted when no longer needed.
...
The override is how we make the code optimistically render the player entity in the new place during that time period.
...
```sh copy
git reset --hard 21a584661cef2fd956acf13e534d3fa8cd2a26c1
```

...
1. Create this script in `script/MessagingExtension.s.sol`.
   ...
   contract MessagingExtension is Script {
     function run() external {
       ...
       IStore(worldAddress).registerTable(
         MessagesTableId,
         Messages.getFieldLayout(),
         Messages.getKeySchema(),
         Messages.getValueSchema(),
         Messages.getKeyNames(),
         Messages.getFieldNames()
       );
...
1. You can see in the user interface of `extendMe` that the counter has been incremented.
   ...
   The next 320 characters are words 0-4, which are not the actual message (each word is 32 bytes, which is 64 hexadecimal digits). We also don't need the trailing newline. `${RAW_RESULT : 322 : -2}` removes those characters so we can use [`cast`]() to get the ASCII.

   So we cut the first 322 characters with `tail -c +322` and then we cut the trailing newline with

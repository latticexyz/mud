...
Any access from the `System` to [the tables that store the data](/world/tables) also goes through the `World`, making it impossible to bypass access control.
Those `System` contracts themselves are stateless, all the data in managed by the `World`.
...
1. An entity (either an [externally owned account](https://ethereum.org/en/developers/docs/accounts/#types-of-account) or a contract) sends a request to the `World`.
2. The `World` authorizes the request and sends it to the appropriate `System`.
3. The `System` can read information from all the `World`'s tables, and modify information in tables in its own namespace.

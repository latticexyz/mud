...
- Owners are automatically granted access, which is revoked when ownership is transferred. 
  For the old owner to keep access, the new owner needs to regrant it.
  Note that this is for access to the entire namespace.
  Permissions for specific objects in the namespace are maintained.

...
library RootSystemLib {
  ...
  function setValueInA(RootSystemType self, ASystemThing memory thing) internal {
  ...
  function setValueInA(CallWrapper memory self, ASystemThing memory thing) internal {
    ...
    bytes memory systemCall = abi.encodeWithSelector(bytes4(keccak256("setValueInA((uint256))")), thing.a);
    self.from == address(0)
  ...
  function getValueFromA(CallWrapper memory self) internal view returns (uint256) {
    ...
    bytes memory systemCall = abi.encodeWithSelector(bytes4(keccak256("getValueFromA()")));

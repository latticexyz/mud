  ...
  return {
    transport: http(
      assert(
        ...
        "EntryKit was not configured with `erc4337` and is attempting to set up a default transport, but did not find an `erc4337Bundler.http` URL on `chain.rpcUrls`.\n\nYou can either add that to your chain config or disable ERC-4337 with `erc4337: false`.",
      ),
    ),
    paymasters: [
      {
        ...
        address: assert(
          ...
          // eslint-disable-next-line max-len
          "EntryKit was not configured with `erc4337` and is attempting to set up a default paymaster, but did not find a `gasTank` contract on `chain.contracts`.\n\nYou can either add that to your chain config or disable ERC-4337 with `erc4337: false`.",
        ),
      },
    ],
  };

...
import { entryPointGasSimulationsAbi, entryPointGasSimulationsBytecode } from "../entryPointGasSimulations";
...
async function simulateGas({ request, userOp }: SimulateGasOptions): Promise<SimulateGasResult> {
  ...
  const simulationParams = [
    ...
    "pending",
    {
      ...
      [entryPoint07Address]: {
        code: entryPointGasSimulationsBytecode,
      },
    },
  ];
  const encodedSimulationResult: Hex = await request({
    // TODO: go back to wiresaw_callEntryPointSimulations once bytecode is updated
    method: "wiresaw_call", // "wiresaw_callEntryPointSimulations",

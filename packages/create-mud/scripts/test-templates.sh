...
# directory is outside of this repository. Node.js/TypeScript can resolve modules/types by looking
# up in ancestor directories' `node_modules`. Hence, if a created project is anywhere within this
# repository, it might miss errors.
tmp_dir="$(mktemp -d)"
...
for template_name in "$@"; do
  ...
  pnpm --dir "$project_path" run test
done

 ...
 */
export function noise2d(x: i64, y: i64): i64 {
  const temp: i64 = ptable((x >> 16) & 0xff /* Unit square X */);

  const a: i64 = ptable((temp >> 8) + ((y >> 16) & 0xff) /* Unit square Y */);

  const b: i64 = ptable((temp & 0xff) + ((y >> 16) & 0xff));
  ...
  const c: i64 = lerp(u, grad2(a >> 8, x, y), grad2(b >> 8, x - 0x10000, y));
  const d: i64 = lerp(u, grad2(a & 0xff, x, y - 0x10000), grad2(b & 0xff, x - 0x10000, y - 0x10000));
 ...
 *      and `ptable` were written.
 */
export function noise3d(x: i64, y: i64, z: i64): i64 {
  const scratch: i64[] = [
 ...
 */
function lerp(t: i64, a: i64, b: i64): i64 {
  return a + ((t * (b - a)) >> 12);
}
 ...
 */
function fade(t: i64): i64 {
  const n: i64 = ftable(t >> 8);

  // Lerp between the two points grabbed from the fade table.
  const lower: i64 = n >> 12;
  const upper: i64 = n & 0xfff;
 ...
 */
function grad2(h: i64, x: i64, y: i64): i64 {
  h &= 3;
 ...
 */
function grad3(h: i64, x: i64, y: i64, z: i64): i64 {
  h &= 0xf;

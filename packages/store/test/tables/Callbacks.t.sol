...
import { mudstore_Callbacks } from "../../src/codegen/Tables.sol";

contract CallbacksTest is Test, GasReporter, StoreReadWithStubs {
  function testSetAndGet() public {
    mudstore_Callbacks.registerSchema();
    ...
    mudstore_Callbacks.set(key, callbacks);
    endGasReport();

    startGasReport("get field from Callbacks (warm)");
    bytes24[] memory returnedCallbacks = mudstore_Callbacks.get(key);
    endGasReport();
    ...
    mudstore_Callbacks.push(key, callbacks[0]);
    endGasReport();

    returnedCallbacks = mudstore_Callbacks.get(key);

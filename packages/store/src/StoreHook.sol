...
library StoreHookLib {
  ...
  function encodeBitmap(EnabledStoreHooks memory enabledHooks) internal pure returns (uint8) {
    uint256 bitmap = 0;
    if (enabledHooks.onBeforeSetRecord) bitmap |= 1 << uint8(HookType.BEFORE_SET_RECORD);
    if (enabledHooks.onAfterSetRecord) bitmap |= 1 << uint8(HookType.AFTER_SET_RECORD);
    if (enabledHooks.onBeforeSetField) bitmap |= 1 << uint8(HookType.BEFORE_SET_FIELD);
    if (enabledHooks.onAfterSetField) bitmap |= 1 << uint8(HookType.AFTER_SET_FIELD);
    if (enabledHooks.onBeforeDeleteRecord) bitmap |= 1 << uint8(HookType.BEFORE_DELETE_RECORD);
    if (enabledHooks.onAfterDeleteRecord) bitmap |= 1 << uint8(HookType.AFTER_DELETE_RECORD);
    return uint8(bitmap);
  }
   ...
   */
  function encode(IStoreHook storeHook, EnabledStoreHooks memory enabledHooks) internal pure returns (StoreHook) {

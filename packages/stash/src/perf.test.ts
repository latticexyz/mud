...
describe.skip("setting records in stash", () => {
  ...
  it("[stash]: setting 10 records", () => {
    printDuration("setting 10 records", () => {
      for (let i = 0; i < 10; i++) {
        stash.setRecord({ table: Position, key: { player: `0x${i}` }, record: { x: i, y: i } });
      }
    });
  });
...
describe.skip("setting records in zustand", () => {
  ...
  beforeEach(() => {
    stash = createZustandStore<State & Actions>((set) => ({
      namespaces: {
        app: {
          Position: {
            "0x": {
  ...
  it("[zustand]: setting 10 records", () => {
    printDuration("setting 10 records", () => {
      for (let i = 0; i < 10; i++) {
        stash.getState().setPosition(String(i), { x: i, y: i });
      }
    });
  });
...
describe.skip("setting records in zustand with mutative", () => {
  ...
  beforeEach(() => {
    stash = createZustandStore<State & Actions>()(
      mutative((set) => ({
        namespaces: {
          app: {
            Position: {

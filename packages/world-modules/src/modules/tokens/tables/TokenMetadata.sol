...
library TokenMetadata {
   ...
   */
  function get(ResourceId _tableId) internal view returns (TokenMetadataData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](0);
   ...
   * @notice Get the full data.
   */
  function _get(ResourceId _tableId) internal view returns (TokenMetadataData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](0);
   ...
   */
  function set(ResourceId _tableId, TokenMetadataData memory _table) internal {
    bytes memory _staticData;
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(ResourceId _tableId, TokenMetadataData memory _table) internal {
    bytes memory _staticData;
    PackedCounter _encodedLengths = encodeLengths(_table.name, _table.symbol, _table.baseURI);
    bytes memory _dynamicData = encodeDynamic(_table.name, _table.symbol, _table.baseURI);

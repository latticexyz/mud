...
struct ERC1155MetadataData {
  string name;
  string symbol;
  string baseURI;
}

library ERC1155Metadata {
  /**
   ...
   */
  function get(ResourceId _tableId) internal view returns (ERC1155MetadataData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](0);
   ...
   * @notice Get the full data.
   */
  function _get(ResourceId _tableId) internal view returns (ERC1155MetadataData memory _table) {
    bytes32[] memory _keyTuple = new bytes32[](0);
   ...
   */
  function set(ResourceId _tableId, ERC1155MetadataData memory _table) internal {
    bytes memory _staticData;
   ...
   * @notice Set the full data using the data struct.
   */
  function _set(ResourceId _tableId, ERC1155MetadataData memory _table) internal {
    bytes memory _staticData;
    PackedCounter _encodedLengths = encodeLengths(_table.name, _table.symbol, _table.baseURI);
    bytes memory _dynamicData = encodeDynamic(_table.name, _table.symbol, _table.baseURI);
  ...
  ) internal pure returns (ERC1155MetadataData memory _table) {

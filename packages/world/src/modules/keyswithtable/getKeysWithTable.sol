 ...
 */
function getKeysWithTable(bytes32 tableId) view returns (bytes32[] memory keysWithTable) {
  // Get the corresponding reverse mapping table
  bytes32 keysWithTableTableId = getTargetTableSelector(MODULE_NAMESPACE, tableId);
 ...
 */
function getKeysWithTable(IStore store, bytes32 tableId) view returns (bytes32[] memory keysWithTable) {
  // Get the corresponding reverse mapping table
  bytes32 keysWithTableTableId = getTargetTableSelector(MODULE_NAMESPACE, tableId);

...
function getMessageHash() pure returns (bytes32) {
  return keccak256("");
}
...
contract WorldRegistrationSystem is System, IWorldErrors, LimitedCallContext {
  ...
  function registerDelegationWithSignature(bytes memory signature) public onlyDelegatecall {
    bytes32 messageHash = getMessageHash();
    bytes32 ethSignedMessageHash = getEthSignedMessageHash(messageHash);

    require(recoverSigner(ethSignedMessageHash, signature) == msg.sender, "not valid");
  }

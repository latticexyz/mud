...
library SystemRouteSchemaLib {
  /** Get the table's schema */
  function getSchema() internal pure returns (Schema schema) {
    schema = SchemaLib.encode(SchemaType.UINT256);
  }
  ...
  function set(
    ...
    keyTuple[0] = bytes20(system);
    StoreSwitch.setField(tableId, keyTuple, 0, abi.encodePacked(value));
  }

  /** Get the table's data */
  function get(uint256 tableId, address system) internal view returns (uint256) {
    bytes32[] memory keyTuple = new bytes32[](1);
    keyTuple[0] = bytes20(system);
  ...
  function has(uint256 tableId, address system) internal view returns (bool) {
    bytes32[] memory keyTuple = new bytes32[](1);
    keyTuple[0] = bytes20(system);

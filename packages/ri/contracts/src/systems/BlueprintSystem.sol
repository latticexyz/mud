// SPDX-License-Identifier: Unlicense
pragma solidity >=0.8.0;
import { ISystem } from "solecs/interfaces/ISystem.sol";
import { IWorld } from "solecs/interfaces/IWorld.sol";
import { IUint256Component } from "solecs/interfaces/IUint256Component.sol";
import { getAddressById } from "solecs/utils.sol";

import { GameConfigComponent, ID as GameConfigComponentID, GameConfig, GodID } from "../components/GameConfigComponent.sol";

uint256 constant ID = uint256(keccak256("ember.system.blueprint"));

contract BlueprintSystem is ISystem {
  IUint256Component components;
  IWorld world;
  address owner;

  constructor(IUint256Component _components, IWorld _world) {
    components = _components;
    world = _world;
    owner = msg.sender;
  }

  function requirement(bytes memory) public view returns (bytes memory) {
    require(msg.sender == owner, "only owner can create entities from blueprints");
  }

  // Create a new entity from a blueprint
  function execute(bytes memory arguments) public returns (bytes memory) {
    requirement(arguments);
  }

  function requirementTyped() public view returns (bytes memory) {
    return requirement(new bytes(0));
  }

  function executeTyped() public returns (bytes memory) {
    return execute(new bytes(0));
  }

  // ------------------------
  // Internals
  // -----------------------

  function createFromSoldierBlueprint(uint256 ownerId) private returns (uint256 entity) {
    entity = world.getUniqueEntityId();
    // TODO: ignore for now during rebase, will be fixed in the next PR
  }

  function createSoldierBlueprint() public {
    // TODO: ignore for now during rebase, will be fixed in the next PR
  }
}

...
async function main() {
  ...
  if (!worldAddress) {
    ...
    if (!worldAddress) {
      throw new Error(
        "World address not provided and not found in worlds.json. Use --worldAddress to specify a world, or set different path for worlds configuration file with --worldsFile.",
      );
    }
  }

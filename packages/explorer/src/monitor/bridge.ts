...
export type BridgeEnvelope = { mud: "explorer/monitor"; data: unknown };

export function isBridgeEnvelope(input: unknown): input is BridgeEnvelope {
  return (
    typeof input === "object" && input !== null && "mud" in input && input.mud === "explorer/monitor" && "data" in input
  );
}

export function wrapMessage(data: unknown): BridgeEnvelope {
  return { mud: "explorer/monitor", data: data };
}
...
export function createBridge({ url, timeout = 10_000 }: CreateBridgeOpts): EmitMessage {
  ...
  iframe.addEventListener(
    "error",
    (error) => {
      debug("monitor iframe error", error);
      emit.reject(error);
    },
    { once: true },
  );
  ...
  wait(timeout).then(() => {
    emit.reject(new Error("Timed out waiting for monitor iframe to load."));
  });

  debug("mounting monitor iframe", url);
  iframe.src = url;

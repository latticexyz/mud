...
export async function ensureDeployer(client: Client<Transport, Chain | undefined, Account>): Promise<Address> {
  ...
  if (gasNeeded > 0) {
    ...
    const gasReceipt = await getAction(client, waitForTransactionReceipt, "waitForTransactionReceipt")({ hash: gasTx });
  ...
  const deployTx = await getAction(
    ...
    sendRawTransaction,
    "sendRawTransaction",
  )({ serializedTransaction: `0x${deployment.transaction}` }).catch((error) => {
  ...
  const deployReceipt = await getAction(
    client,

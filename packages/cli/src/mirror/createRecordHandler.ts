import { Account, Address, Chain, Client, Hex, Transport, encodeFunctionData, formatEther, size, zeroHash } from "viem";
...
export function createRecordHandler({
  ...
  async function onBatch(batch: StoreRecord[]) {
    ...
    const estimatedCalldata = Math.ceil((status.calldataSent / status.submitted) * totalRecords);
